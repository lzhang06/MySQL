...
get_ipython().run_cell_magic('sql', '', 'SELECT DISTINCT d.dog_guid, d.dimension, COUNT(c.created_at)\nFROM dogs d, complete_tests c\nWHERE d.dog_guid = c.dog_guid\nGROUP BY d.dog_guid\nLIMIT 100')
...
get_ipython().run_cell_magic('sql', '', 'SELECT NUMTESTS.dimension AS dimension, AVG(NUMTESTS.NUM)\nFROM (SELECT DISTINCT d.dog_guid, d.dimension, COUNT(c.created_at) AS NUM\n     FROM dogs d, complete_tests c\n     WHERE d.dog_guid = c.dog_guid\n     GROUP BY d.dog_guid) AS NUMTESTS\nGROUP BY dimension')
...
get_ipython().run_cell_magic('sql', '', 'SELECT NUM.Dimension, NUM.Num\nFROM (SELECT COUNT(DISTINCT d.dog_guid) AS Num, d.dimension AS Dimension\n     FROM dogs d, complete_tests c\n     WHERE d.dog_guid = c.dog_guid\n     GROUP BY d.dimension\n     ) AS NUM\nWHERE NUM.Dimension IS NULL OR NUM.Dimension = ""')


# In[21]:

get_ipython().run_cell_magic('sql', '', "SELECT dimension, COUNT(dogID) AS num_dogs\nFROM (SELECT d.dog_guid AS dogID, d.dimension AS dimension\nFROM dogs d JOIN complete_tests c\nON d.dog_guid=c.dog_guid\nWHERE d.dimension IS NULL OR d.dimension=''\nGROUP BY dogID) AS dogs_in_complete_tests\nGROUP BY dimension;")
...
get_ipython().run_cell_magic('sql', '', 'SELECT d.breed, d.weight, d.exclude, MIN(c.created_at) AS first_test, MAX(c.created_at) AS last_test\nFROM dogs d JOIN complete_tests c\nON d.dog_guid = c.dog_guid\nWHERE d.dimension = ""\nGROUP BY d.dog_guid')
...
get_ipython().run_cell_magic('sql', '', 'SELECT NUMTESTS.dimension AS dimension, AVG(NUMTESTS.NUM), COUNT(DISTINCT NUMTESTS.dog_guid)\nFROM (SELECT DISTINCT d.dog_guid, d.dimension, COUNT(c.created_at) AS NUM\n     FROM dogs d JOIN complete_tests c\n     ON d.dog_guid = c.dog_guid \n     WHERE (d.dimension != "" AND d.dimension IS NOT NULL) AND (d.exclude IS NULL OR d.exclude = 0)\n     GROUP BY d.dog_guid) AS NUMTESTS\nGROUP BY dimension')
...
get_ipython().run_cell_magic('sql', '', 'SELECT DISTINCT d.breed_group AS breed_group, COUNT(c.created_at) AS NUM\nFROM dogs d JOIN complete_tests c\nON d.dog_guid = c.dog_guid\nWHERE (d.dimension != "" AND d.dimension IS NOT NULL) AND (d.exclude IS NULL OR d.exclude = 0)\nGROUP BY breed_group')


# In[52]:

get_ipython().run_cell_magic('sql', '', 'SELECT breed_group, AVG(numtests_per_dog.numtests) AS avg_tests_completed,\nCOUNT(DISTINCT dogID)\nFROM( SELECT d.dog_guid AS dogID, d.breed_group AS breed_group,\ncount(c.created_at) AS numtests\nFROM dogs d JOIN complete_tests c\nON d.dog_guid=c.dog_guid\nWHERE d.exclude IS NULL OR d.exclude=0\nGROUP BY dogID) AS numtests_per_dog\nGROUP BY breed_group;')
...
get_ipython().run_cell_magic('sql', '', 'SELECT NUMTESTS.breed_type AS breed_type, AVG(NUMTESTS.NUM), COUNT(DISTINCT NUMTESTS.dog_guid)\nFROM (SELECT DISTINCT d.dog_guid, d.breed_type, COUNT(c.created_at) AS NUM\n     FROM dogs d JOIN complete_tests c\n     ON d.dog_guid = c.dog_guid \n     WHERE d.exclude IS NULL OR d.exclude = 0\n     GROUP BY d.dog_guid) AS NUMTESTS\nGROUP BY breed_type')
...
get_ipython().run_cell_magic('sql', '', 'SELECT DISTINCT d.dog_guid AS ID, d.breed_type, COUNT(c.created_at),\nCASE WHEN d.breed_type = "Pure Breed" THEN "Pure_Breed"\nELSE "Pure_Breed" \nEND AS BREED_TYPE\nFROM dogs d, complete_tests c\nWHERE d.dog_guid = c.dog_guid\nGROUP BY ID\nLIMIT 50')
...
get_ipython().run_cell_magic('sql', '', 'SELECT NUMTESTS.PURE_NONPURE AS breed_type, COUNT(DISTINCT NUMTESTS.dog_guid)\nFROM (SELECT DISTINCT d.dog_guid, COUNT(c.created_at) AS NUM, \n      CASE WHEN d.breed_type = "Pure Breed" THEN "Pure_Breed"\n      ELSE "non_Pure_Breed"\n      END AS PURE_NONPURE\n     FROM dogs d JOIN complete_tests c\n     ON d.dog_guid = c.dog_guid \n     WHERE d.exclude IS NULL OR d.exclude = 0\n     GROUP BY d.dog_guid) AS NUMTESTS\nGROUP BY breed_type')
...
get_ipython().run_cell_magic('sql', '', 'SELECT NUMTESTS.PURE_NONPURE AS breed_type,Neutered, COUNT(DISTINCT NUMTESTS.dog_guid) AS NUM1\nFROM (SELECT DISTINCT d.dog_guid, d.dog_fixed AS Neutered, COUNT(c.created_at) AS NUM, \n      CASE WHEN d.breed_type = "Pure Breed" THEN "Pure_Breed"\n      ELSE "non_Pure_Breed"\n      END AS PURE_NONPURE\n     FROM dogs d JOIN complete_tests c\n     ON d.dog_guid = c.dog_guid \n     WHERE d.exclude IS NULL OR d.exclude = 0\n     GROUP BY d.dog_guid) AS NUMTESTS\nGROUP BY breed_type, Neutered')


# In[69]:

get_ipython().run_cell_magic('sql', '', 'SELECT DISTINCT dog_fixed\nFROM dogs')
...
get_ipython().run_cell_magic('sql', '', 'SELECT NUMTESTS.dimension AS dimension, AVG(NUMTESTS.NUM), COUNT(DISTINCT NUMTESTS.dog_guid), STDDEV(NUMTESTS.NUM)\nFROM (SELECT DISTINCT d.dog_guid, d.dimension, COUNT(c.created_at) AS NUM\n     FROM dogs d JOIN complete_tests c\n     ON d.dog_guid = c.dog_guid \n     WHERE (d.dimension != "" AND d.dimension IS NOT NULL) AND (d.exclude IS NULL OR d.exclude = 0)\n     GROUP BY d.dog_guid) AS NUMTESTS\nGROUP BY dimension')
...
get_ipython().run_cell_magic('sql', '', 'SELECT d.breed_type, AVG(TIMESTAMPDIFF(minute, e.start_time, e.end_time)) AS DURATION, \nSTDDEV(TIMESTAMPDIFF(minute, e.start_time, e.end_time)) AS STD_DURATION\nFROM dogs d LEFT JOIN exam_answers e\nON d.dog_guid = e.dog_guid\nWHERE TIMESTAMPDIFF(minute, e.start_time, e.end_time) >0\nGROUP BY d.breed_type')
